...
export async function listTodos(): Promise<Todo[]> {
  ...
  return todos.filter((todo): todo is Todo => Boolean(todo)).map((todo) => todoSchema.parse(todo));
}
export async function createTodo(text: string): Promise<Todo> {
  const todo: Todo = { id: crypto.randomUUID(), text, completed: false, urgent: false, createdAt: new Date().toISOString() };
  const redis = getRedis();
  await Promise.all([redis.hset(ITEMS_KEY, { [todo.id]: todo }), redis.zadd(ORDER_KEY, { score: Date.now(), member: todo.id })]);
  return todo;
}
export async function updateTodo(id: string, changes: Partial<Pick<Todo, "text" | "completed" | "urgent">>) {
  const redis = getRedis(); const current = await redis.hget<Todo>(ITEMS_KEY, id); if (!current) return null;
  const updated = { ...todoSchema.parse(current), ...changes }; await redis.hset(ITEMS_KEY, { [id]: updated }); return updated;
}
