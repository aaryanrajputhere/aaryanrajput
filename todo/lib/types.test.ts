...
import { createTodoSchema, importTodosSchema, updateTodoSchema } from "./types";
...
describe("todo validation", () => {
  ...
  it("accepts a bounded import with valid records", () => {
    expect(importTodosSchema.safeParse({ todos: [{ id: crypto.randomUUID(), text: "saved", completed: false, createdAt: new Date().toISOString() }] }).success).toBe(true);
  });
});
